<template>
    <div class="w-full shadow-md">
        <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
            <div>
                <button @click="toggleDrawer" type="button" class="bg-blue-700 hover:bg-blue-800 text-white focus:ring-4 focus:ring-blue-300 font-medium rounded-sm text-sm px-5 py-2 ml-3 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Create</button>
            </div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="service in filteredServices" :key="service.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hoveur:bg-gray-600">
                    ...
                    <td class="border px-6 py-4">{{ service.id }}</td>
                    <td class=" border px-6 py-4">{{ service.name }}</td>
                    <td class="border px-6 py-4">{{ service.description }}</td>
                    <td class="border px-6 py-4">{{ service.price }}</td>
                    <td v-if="service.imageUrl" class="border px-6 py-4">
                        <img :src="service.imageUrl" alt="service image" class="w-10 h-10 object-cover">
                    </td>
                    <td class="border px-6 py-4">{{ formatDateTime(service.createdAt) }}</td>
                    <td class="border px-6 py-4">{{ formatDateTime(service.updatedAt) }}</td>
                    <td class="px-6 py-4 flex flex-row items-center justify-center gap-2">
                        <span @click="$router.push({name: 'Addon', params: {id: service.id}})" class="material-symbols-outlined cursor-pointer text-blue-600">visibility</span>
        ...
        <RightDrawer :drawerOpen="isDrawerOpen" @toggle="toggleDrawer">
                    <div class="flex justify-center">
                        <h5 class="mb-6 text-base font-semibold text-gray-500 uppercase dark:text-gray-400">Enter Service Data</h5>
                    </div>
                    <form @submit.prevent="createService" class="mb-6">
                        <div class="mb-6">
                            ...
                            <input v-model="serviceData.name" type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Home Cleaning" required />
                        </div>
                        <div class="mb-6">
                            <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Price</label>
                            <input v-model="serviceData.price" type="text" id="price" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="1000.00" required />
                        </div>
                        
                        <div class="flex items-center justify-center w-full">
                            <label v-if="!serviceData.imageUrl" for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea v-model="serviceData.description" id="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Service Description.."></textarea>
                        </div>
                        <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 w-full focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 block">Create Service</button>
                    </form>
        </RightDrawer>
...
const filteredServices = computed(() => {
    return genericFilter(serviceStore.services, query.value, ['name', 'description', 'id', 'price'])
})
