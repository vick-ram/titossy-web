...
export const useEmployeeStore = defineStore("employee", {
  ...
  actions: {
    ...
    async signup() {
        this.loading = true
      try {
        const res = await post<ApiResponse<Employee>>(
          "/employee/auth/sign_up",
          this.employeeData
        );
    ...
    async signin() {
      try {
        const res = await post<ApiResponse<String>>(
          "/employee/auth/sign_in",
          this.employeeLoginData
        );
        if (res.data.status === "success") {
          if (res.data.data) {
            this.successMessage = res.data.message as string;
    ...
    async updateStatus(id: string, status: string) {
      try {
        const res = await patch<ApiResponse<null>>(`/employee/${id}/status`, {
          status,
        });
        if (res.data.status === "success") {
          this.successMessage = res.data.message as string;
